...
const paraContainer=document.getElementById("data");


function generate(){

    if(isNaN(input.value) || input.value<0 || input.value>9){
        const ran=Math.floor(Math.random()*text.length);
        paraContainer.innerHTML=`<p> ${text[ran]} </p>`;
    }
    else{

        const myText=shuffle(text);

        const dataArr=myText.slice(0,input.value);

        const paraData= dataArr.map(
            function(para){
                return `<p>${para}</p>`;
            }
        )

        paraContainer.innerHTML=paraData.join(" ");
    }
}